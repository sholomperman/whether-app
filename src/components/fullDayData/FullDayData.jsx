...
const FullDayData = ({isC, display, setDisplay, hoursData, select }) => {
  ...
  return (
      <div
          className="fullDayData"
          onClick={() => setDisplay(!display)}
          style={{
              display: display ? 'none' : 'block',
              background: (select?.weather.code >= 200 && select?.weather.code < 800) ||
                  select?.weather.code === 900 ? rain : select?.weather.code >= 800 && select?.weather.code <= 802 ? sunny : select?.weather.code >= 803 && select.weather.code <= 804 ? cloudy : null
          }}>
    <div className="card">
        <div className="about">
